...
print("Uploading modified file to tmpfiles.org...")
try:
    # We name the file as the original retrieved name
    ...
    if r.status_code == 200:
        res = r.json()
        print("\n--- TMPFILES.ORG UPLOAD SUCCESS ---")
        # Direct URL conversion
        raw_url = res['data']['url']
        direct_url = raw_url.replace("https://tmpfiles.org/", "https://tmpfiles.org/dl/")
        print("View Link:", raw_url)
        print("Direct Download Link:", direct_url)
    else:
        print("tmpfiles.org upload failed with status:", r.status_code)
except Exception as e:
    print("tmpfiles.org upload raised exception:", e)

# Upload to file.io (fallback)
print("\nUploading modified file to file.io...")
try:
    r = requests.post(
        "https://file.io",
        files={"file": (filename, file_bytes)},
    ...
    if r.status_code == 200:
        res = r.json()
        print("\n--- FILE.IO UPLOAD SUCCESS ---")
        print("Download Link:", res['link'])
    else:
        print("file.io upload failed with status:", r.status_code)
except Exception as e:
    print("file.io upload raised exception:", e)
